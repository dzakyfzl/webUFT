...
/**
 * Fetches active partners from the backend.
 * Backend already filters is_active=true and sorts: sponsor → media_partner → urutan ASC.
 * Maps to CarouselPartner[] for the landing page carousel.
 */
export async function fetchPartnersData(): Promise<CarouselPartner[]> {
  if (USE_FAKE_DATA) {
    console.info("[api] NEXT_PUBLIC_USE_FAKE_DATA=true — menggunakan fake partner data.");
    return [];
  }

  try {
    const res = await fetch("/api/partner/ambil-semua");
    if (!res.ok) {
      // Non-critical: jika gagal, kembalikan array kosong agar halaman tetap tampil
      console.warn(`[api] Gagal mengambil data partner (status ${res.status})`);
      return [];
    }
    const partners: PartnerBackend[] = await res.json();
    ...
    return partners.map((p) => ({
      ...
      image: p.fileID ? `/api/file/ambil/${p.fileID}` : "",
      kategori: p.kategori,
    }));
  } catch (err) {
    console.warn("[api] Error fetching partners:", err);
    return [];
  }
}
